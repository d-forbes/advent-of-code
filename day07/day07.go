...
func Day07() {
	lines, _ := os.ReadFile("day07/day07input.txt")
	start := time.Now()
	fmt.Println("Day 07 Part A Result:", partA(string(lines)), "in", time.Since(start))
	start = time.Now()
	fmt.Println("Day 07 Part B Result:", partB(string(lines)), "in", time.Since(start))
}
