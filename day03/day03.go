...
func Day03() {
	file, err := os.ReadFile("day03/day03Input.txt")
	if err != nil {
		panic(err)
	}
	...
	start := time.Now()
	fmt.Println("Day 03 Part A Result:", day03a(lines, gears), "in", time.Since(start))
	start1 := time.Now()
	fmt.Println("Day 03 Part B Result:", day03b(gears), "in", time.Since(start1))

}

func day03a(lines []string, gears map[string][]int) int {
...
func day03b(gears map[string][]int) int {
	total := 0
	for _, v := range gears {
		if len(v) == 2 {
			total += v[0] * v[1]
